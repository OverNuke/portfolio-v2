...
**Phase 1 (React + Vite + TS + Tailwind setup), Phase 2 (app shell/panel system/navigation), and Phase 3's Collage Canvas are all done**, per the archived SDD change `phase2-app-shell` (2026-07-29). `src/App.tsx` composes `TurnProvider` → an `AppShell` that renders `NameRevealIntro` (gated once at mount to whether the initial route is `/`), `Shell` (= Home, wrapping the collage `Canvas` — a real 12×12 grid with real `NavItem` buttons), and a `PageLayer` for the active route among `/profile`, `/projects`, `/skills`, `/contact`. Page-turn navigation (`src/turn/`) drives focus and `inert` on route change.

What's still actually missing: real Profile/Projects/Skills/Contact page **content** (the routed pages under `src/routes/` still render placeholder text — that's Phase 3's module content, not yet started); roadmap Phase 4 (textures, animations, advanced effects); and the broader roadmap Phase 5 accessibility audit (the automated collage audit — `scripts/audit.mjs` — and CI now cover the page-turn/collage slice of this; a full a11y audit pass is still open).
...
- `.Codex/`, `.atl/`, `.impeccable/` are gitignored tool state. `Codex/` (no leading dot) is NOT — it is a spec location cited by live code (`src/components/project-sheet/project-sheet.css` line 3). Never sweep it.
...
- **No scrolling at the shell level.** Home is a fixed single-viewport dashboard (`100dvh`, not `100vh`). Opening a module/project/page is a real state change ("page turn"), not scroll — each page gets its own route (`/`, `/profile`, `/projects`, `/skills`, `/contact`) so browser back/forward and deep-links work. A page's *own content* may scroll internally; the shell never does. Below 768px the no-scroll rule is released per WCAG 1.4.10 reflow (accessibility beats vision, per the conflict order above).
...
- **Decorative auto-playing animations must be fully `aria-hidden`, never partially.** `src/components/name-reveal-intro/` is the reference implementation and post-mortem for this pattern (see its README and inline comments): a rapidly auto-cycling animation has no coherent screen-reader translation, so it's marked fully decorative, never receives focus, and dismisses on *any* keypress — while the host page's real content must independently carry the same information (name/role) in accessible markup. Follow this same pattern for any future intro/loading/section-sting animation.

## Existing code

`src/components/name-reveal-intro/` is now wired into `App.tsx` (gated to initial `/` loads only, per the accessibility pattern above), but it's still self-themed via CSS custom properties in `NameRevealIntro.css` rather than Tailwind, using `motion` (the framer-motion successor package) and `gsap`. Treat it as the reference for: how theming-via-CSS-variables should look, and how the accessibility pattern above is implemented in practice. Its README documents props and behavior in full.
