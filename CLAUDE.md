...
**Phase 1 (React + Vite + TS + Tailwind setup), Phase 2 (app shell/panel system/navigation), and Phase 3's Collage Canvas are all done**, per the archived SDD change `phase2-app-shell` (2026-07-29). `src/App.tsx` composes `TurnProvider` → an `AppShell` that renders `Shell` (= Home, wrapping the collage `Canvas`), and a `PageLayer` for the active route among `/profile`, `/certifications`, `/projects`, `/contact`. Page-turn navigation (`src/turn/`) drives focus and `inert` on route change. (The `NameRevealIntro` cinematic overlay that used to gate `Shell`'s first render at `/` was removed per the archived SDD change `drop-intro-hero-placeholder` (2026-08-24) — Home now paints content on the first frame, for every route, with no gating overlay.)

**Corrected 2026-09-15** (`sdd/design-canvas-reconcile-v2`, W1 design): `Canvas.tsx` is a 4-area editorial grid, not a 12×12 grid, and contains **no `NavItem` buttons** — cross-route navigation is the global `ModuleWheel`, not anything inside the collage. `collage.css`, `skills-collage.css`, `SkillsCollage.tsx`, `collageSeeds.ts`, `useCollageSeed.ts` and `components/skill-badge/` are dead, unreferenced code (nothing imports them; `src/shell/collage/collage.test.ts` itself flags them for a future cleanup pass) — don't build on them without first checking whether they're still on disk.

What's still actually missing: real Profile/Certifications/Projects/Contact page **content** beyond what's already shipped (the routed pages under `src/routes/` still render placeholder text for anything not yet reconciled against the design canvas); roadmap Phase 4 (textures, animations, advanced effects); and the broader roadmap Phase 5 accessibility audit (the automated collage audit — `scripts/audit.mjs` — and CI now cover the page-turn/collage slice of this; a full a11y audit pass is still open). There is no `/skills` route yet — Skills is scoped as Wave 4 of `sdd/design-canvas-reconcile-v2`.
