...
- Hard-cut easing, matching `--ease-hard`
  ...
  easing unchanged. The original 140ms was tuned for a text-height wipe
  inside the now-deleted `NameRevealIntro` component (removed
  `sdd/drop-intro-hero-placeholder`, 2026-08-24 — see
  `src/styles/tokens.css`'s Motion block for the literal values'
  provenance); across a full 1440px viewport it read as a dropped frame
  and the crease direction never registered. See `src/styles/tokens.css`'s
  `--dur-turn`.)
...
most) — consistent with "respect prefers-reduced-motion" below.
