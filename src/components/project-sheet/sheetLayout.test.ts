...
import { assignSlots, clampSheet, paginate, RECORDS_PER_SHEET, templateFor } from "./sheetLayout";
...
describe("templateFor", () => {
  it.each([
    ...
    [4, 3],
    [5, 4],
  ])("%i records selects template T%i", (records, template) => {
    expect(templateFor(records)).toBe(template);
  });

  it("selects the reference layout for the current record count", () => {
    expect(templateFor(PROJECTS.length)).toBe(2);
  });
});
