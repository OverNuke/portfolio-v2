 ...
 *
 * The identity (name/role) content lives here UNCONDITIONALLY. Home's
 * `<main>` must carry the name and role as real accessible content because
 * everything painted on the collage canvas (masthead, KG initials, the
 * reserved hero slot) is `aria-hidden` — this is the ONLY accessible
 * carrier of that information on Home (re-grounded
 * `sdd/drop-intro-hero-placeholder`, 2026-08-24, after `NameRevealIntro`'s
 * removal; previously framed as satisfying the intro's own contract — see
 * `docs/05_ACCESSIBILITY.MD` for that history). On a deep-link initial
 * load the shell (and this content) synchronously turns `inert` in the
 * same commit as the forward-home turn starts (design's driver effect) —
 * expected: `PageLayer`'s own heading immediately becomes the live
 * accessible surface. The content still structurally exists in the shell
 * throughout.
 *
