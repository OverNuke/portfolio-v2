...
export interface FieldKeyboardOptions {
  /** Called for ArrowLeft when a next field exists. */
  onForward?: () => void;
  /** Called for ArrowRight when a previous field exists. */
  onBack?: () => void;
}
 ...
 * Paging between fields is just those two words with more than one step to
 * unwind, so this hook adds no new binding — it only gets first refusal on
 * the two keys the shell already owns:
 *
 *   Left   next field, if there is one    else falls through (does nothing)
 *   Right  previous field, if on 2+       else falls through -> Home
 *   Esc    never handled here             always falls through -> Home
 *
 * Escape is the deliberate asymmetry: Right unwinds one sheet at a time,
 * Escape exits the module outright. Someone on field 5 should not have to
 ...
 */
export function useFieldKeyboard({ onForward, onBack }: FieldKeyboardOptions): void {
  useEffect(() => {
